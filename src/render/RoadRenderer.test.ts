...
describe("RoadRenderer", () => {
  it("draws all same-level borders before surfaces so connected nodes have no divider seam", () => {
    ...
    expect(groundLayer?.children[0]?.children).toHaveLength(1);
    expect(groundLayer?.children[1]?.children).toHaveLength(1);
  ...
  it("highlights equal non-empty names across different internal road ids", () => {
    ...
    const logical = new RoadRenderer().render(city, { kind: "road", id: "first", edgeId: "first-edge", scope: "logical" });
    expect(logical.children).toHaveLength(4);
    logical.destroy({ children: true });
    const segment = new RoadRenderer().render(city, { kind: "road", id: "first", edgeId: "first-edge", scope: "segment" });
    expect(segment.children).toHaveLength(4);
    segment.destroy({ children: true });
    const editable = new RoadRenderer().render(city, { kind: "road", id: "first", edgeId: "first-edge", scope: "logical" }, true);
    expect(editable.children).toHaveLength(8);
    editable.destroy({ children: true });
  });

  it("highlights an explicit multi-segment selection and exposes its nodes only while editing", () => {
    const city: City = {
      id: "multi", name: "Multi", bounds: { x: 0, y: 0, width: 500, height: 200 }, mapSize: "small", terrain: "flat",
      roadNodes: [{ id: "a", x: 0, y: 0 }, { id: "b", x: 100, y: 0 }, { id: "c", x: 200, y: 0 }], roads: [{ id: "road", category: "normal", subtype: "small", width: 8, name: "Road", segmentIds: ["ab", "bc"] }], roadEdges: [{ id: "ab", roadId: "road", name: "Road", startNodeId: "a", endNodeId: "b", structure: "ground", level: 0, geometry: { type: "line" } }, { id: "bc", roadId: "road", name: "Road", startNodeId: "b", endNodeId: "c", structure: "ground", level: 0, geometry: { type: "line" } }], buildings: [], blocks: [], zones: [], parks: [], districts: [], waters: [], pois: [], facilities: [], universities: [], hospitals: [], companies: [], transitLines: [], transitStations: [], busTerminals: [], busLines: [], busStops: [], labels: [],
    };
    const selection = { kind: "road-multi" as const, edgeIds: ["ab", "bc"], nodeIds: ["b"] }; const normal = new RoadRenderer().render(city, selection); expect(normal.children).toHaveLength(4); normal.destroy({ children: true }); const editable = new RoadRenderer().render(city, selection, true); expect(editable.children).toHaveLength(7); editable.destroy({ children: true });
  });
});
