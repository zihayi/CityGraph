import { Bot, CircleDollarSign, Clock3, Database, FileUp, FolderClock, FolderOpen, Keyboard, Languages, Map, Maximize2, Music2, Plus, RefreshCw, Save, SaveAll, SlidersHorizontal, Sparkles, Volume2, X } from "lucide-react";
...
function formatSaveTime(value: string, locale: Locale): string {
    ...
    <div className="settings-layout">
      ...
      <main className="settings-pane">
        {tab === "saves" && <>
          ...
          <section className="save-library" aria-busy={props.savesLoading}><h4>{props.t("settings.saveTimeline")}</h4>{props.savesLoading ? <div className="save-library-empty">{props.t("settings.loadingSaves")}</div> : props.saves.length === 0 ? <div className="save-library-empty"><FolderClock size={28}/><b>{props.t("settings.noSaves")}</b><span>{props.t("settings.noSavesHint")}</span></div> : <div className="save-timeline">{props.saves.map((slot) => <article key={slot.folderName}><i/><button type="button" onClick={() => props.onLoadSave(slot.folderName)}><span className="save-thumbnail">{slot.thumbnail ? <img src={slot.thumbnail} alt=""/> : <Map size={28}/>}</span><span className="save-copy"><span><b>{slot.mapName}</b>{slot.autosave && <em>{props.t("settings.autosaveBadge")}</em>}</span><small>{slot.saveName}</small><time dateTime={slot.updatedAt}>{formatSaveTime(slot.updatedAt, props.locale)}</time></span></button></article>)}</div>}</section>
