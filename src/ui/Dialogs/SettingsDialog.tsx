...
export function SettingsDialog(props: Props) {
  ...
  return <div className="modal-backdrop settings-backdrop"><section className="dialog-card settings-dialog" role="dialog" aria-modal="true" aria-labelledby="settings-title">
    <header className="settings-header"><div><h2 id="settings-title">{props.t("settings.title")}</h2><small>{props.t("settings.appliedImmediately")}</small></div><button type="button" aria-label={props.t("common.close")} onClick={props.onClose}><X size={19}/></button></header>
    <div className="settings-layout">
      <nav className="settings-sidebar" aria-label={props.t("settings.title")}>{tabs.map(({ id, key, icon: Icon }) => <button key={id} className={tab === id ? "is-active" : ""} type="button" aria-pressed={tab === id} onClick={() => setTab(id)}><Icon size={18}/><span>{props.t(key)}</span></button>)}</nav>
      <main className="settings-pane">
        {tab === "saves" && <>
          ...
          <div className="settings-action-grid"><button className="primary" type="button" onClick={props.onSave}><Save size={16}/><span>{props.t("common.save")}</span></button><button type="button" onClick={props.onSaveAs}><SaveAll size={16}/><span>{props.t("common.saveAs")}</span></button><button type="button" onClick={props.onNew}><Plus size={16}/><span>{props.t("top.newMap")}</span></button><button type="button" onClick={props.onImport}><FileUp size={16}/><span>{props.t("import.title")}</span></button><button type="button" disabled={props.savesLoading || props.saves.length === 0} onClick={() => props.onLoadSave()}><FolderOpen size={16}/><span>{props.t("common.load")}</span></button></div>
