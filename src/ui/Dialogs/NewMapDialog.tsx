...
export function NewMapDialog({ t, onCreate, onCancel }: { t: (key: TranslationKey) => string; onCreate: (options: NewMapOptions) => void; onCancel: () => void }) {
  const [name, setName] = useState(""); const [size, setSize] = useState<MapSize>("medium"); const [terrain, setTerrain] = useState<TerrainType>("flat"); const [lakeCount, setLakeCount] = useState<1|2|3>(2);
  return <div className="modal-backdrop" role="presentation"><section className="dialog-card paper-dialog" role="dialog" aria-modal="true"><header className="dialog-heading"><img src={logoUrl} alt=""/><div><h2>{t("new.title")}</h2><p>{t("new.subtitle")}</p></div></header><div className="dialog-form">
  ...
  </div><footer className="dialog-actions"><button type="button" onClick={onCancel}>{t("common.cancel")}</button><button className="primary" type="button" onClick={() => onCreate({ name: name.trim() || t("home.untitled"), size, terrain, lakeCount })}>{t("common.create")}</button></footer></section></div>;
}
