...
export function NewMapDialog({ t, onCreate, onCancel }: { t: (key: TranslationKey) => string; onCreate: (options: NewMapOptions) => void; onCancel: () => void }) {
  ...
  </div><footer className="dialog-actions"><button type="button" onClick={() => { soundManager.playClick(); onCancel(); }}>{t("common.cancel")}</button><button className="primary" type="button" onClick={() => { soundManager.playClick(); onCreate({ name, size, terrain, lakeCount }); }}>{t("common.create")}</button></footer></section></div>;
}
