...
export function OSMPreview({ city, layers, region, onRegion, disabled, label }: { city: City; layers: OSMLayers; region?: Bounds; onRegion: (bounds: Bounds) => void; disabled: boolean; label: string }) {
  ...
  useEffect(() => {
    ...
    ctx.setTransform(1, 0, 0, 1, 0, 0); ctx.fillStyle = "#f3f3ed"; ctx.fillRect(0, 0, WIDTH, HEIGHT);
    ...
    if (layers.parks) { ctx.fillStyle = "#c8d9bc"; city.parks.forEach((park) => { ctx.beginPath(); ring(park.points); ctx.fill(); }); }
    if (layers.waters) { ctx.fillStyle = "#a9c9d2"; city.waters.forEach((water) => { ctx.beginPath(); ring(water.points); ctx.fill(); }); }
    if (layers.buildings) { ctx.fillStyle = "#b4c0bc"; city.buildings.forEach((building) => { ctx.beginPath(); ring(building.footprint.outer); building.footprint.holes.forEach(ring); ctx.fill("evenodd"); }); }
    if (layers.roads) {
      const nodes = new Map(city.roadNodes.map((node) => [node.id, node])); const roads = new Map(city.roads.map((road) => [road.id, road]));
      ctx.strokeStyle = "#718078"; ctx.lineCap = "round"; ctx.lineJoin = "round";
    ...
    if (layers.facilities) { ctx.fillStyle = "#567b65"; city.facilities.forEach((facility) => { ctx.beginPath(); ctx.arc(facility.position.x, facility.position.y, 2 / scale, 0, Math.PI * 2); ctx.fill(); }); }
  ...
  return <div className="osm-preview-map" onPointerDown={(event) => { if (disabled || event.button !== 0) return; event.preventDefault(); start.current = pointer(event); setDraft(undefined); event.currentTarget.setPointerCapture(event.pointerId); }} onPointerMove={(event) => { if (start.current) setDraft(rectangle(start.current, pointer(event))); }} onPointerUp={(event) => {
    ...
    {box && <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} aria-hidden="true"><path d={`M0 0H${WIDTH}V${HEIGHT}H0Z M${box.x} ${box.y}h${box.width}v${box.height}h${-box.width}Z`} fill="rgba(34,44,36,.22)" fillRule="evenodd"/><rect {...box} fill="none" stroke="#61815a" strokeWidth="2"/></svg>}
  </div>;
}
