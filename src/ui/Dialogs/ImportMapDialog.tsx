  ...
  return <div className="modal-backdrop" style={pickingPosition ? { display: "none" } : undefined}><section ref={dialog} className="dialog-card osm-import-dialog paper-dialog" data-source={source} role="dialog" aria-modal="true" aria-labelledby="osm-import-title" onKeyDown={(event) => {
    ...
    <div className="dialog-form osm-import-body" aria-busy={loading || creating}>
      ...
      {source === "online" ? <OSMDownloadPanel locale={locale} layers={layers} onLayers={setLayers} onBusy={setDownloadBusy} onReady={(downloaded, mapName) => { setResult(downloaded); setName(mapName); setFileName(`${mapName}.osm`); setRegion(undefined); setError(undefined); setSource("file"); }} t={t}/> : <>
        ...
        <details className="osm-import-help"><summary>{t("import.fileSources")}</summary><p>{t("import.downloadHint")} <a href="https://www.openstreetmap.org/export" target="_blank" rel="noreferrer">OpenStreetMap</a> · <a href="https://extract.bbbike.org/" target="_blank" rel="noreferrer">BBBike</a></p></details>
      </>}
