import { Activity, Circle, Copy, Footprints, GitBranch, Grid3X3, Hexagon, Magnet, MousePointer2, Route, RotateCw, Spline, Waves, Waypoints } from "lucide-react";
...
export function RoadToolPalette({ t }: { t: (key: TranslationKey) => string }) {
  ...
  return <aside className="road-palette glass-panel" aria-label={t("tools.roads")}>
    ...
    {store.roadStructure === "ground" && <label className="palette-check"><Waves size={15}/><span><input type="checkbox" checked={store.roadAllowWaterCrossing} onChange={(event) => store.setRoadAllowWaterCrossing(event.target.checked)}/>{t("road.allowWaterCrossing")}</span></label>}
