...
export function LeftToolbar({ currentTool, collapsed, onToolChange, onToggleCollapsed, t }: { currentTool: EditorTool; collapsed: boolean; onToolChange: (tool: EditorTool) => void; onToggleCollapsed: () => void; t: (key: TranslationKey) => string }) {
  return <aside className={`left-toolbar${collapsed ? " is-collapsed" : ""}`}><div className="tool-list" role="toolbar">
    {tools.map(({ id, key, icon: Icon }) => <button key={id} className={currentTool === id ? "is-active" : ""} type="button" title={t(key)} onClick={() => { soundManager.playClick(); onToolChange(id); }}><Icon size={22}/><span className="tool-label">{t(key)}</span></button>)}
  </div><button className="dock-collapse" type="button" title={t(collapsed ? "sidebar.expand" : "sidebar.collapse")} onClick={() => { soundManager.playClick(); onToggleCollapsed(); }}>{collapsed ? <ChevronRight size={18}/> : <><ChevronLeft size={18}/><span>{t("sidebar.collapse")}</span></>}</button></aside>;
}
