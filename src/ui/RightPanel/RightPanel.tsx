...
function FacilityPropertyForm({ editor, facility, t, click }: { editor: Editor; facility: FacilityPOI; t: (key: TranslationKey) => string; click: () => void }) {
  ...
  return <div className="property-form property-view"><label>{t("common.name")}<input value={name} onChange={(event) => setName(event.target.value)} onBlur={() => { if (name !== facility.name) editor.updateFacility(facility.id, { name }); }}/></label><p><b>{t("facility.type")}</b>{facilityTypeName(facility.type, locale)}</p><p><b>{t("facility.position")}</b>{facility.position.x.toFixed(1)}, {facility.position.y.toFixed(1)}</p><button className="danger-action" type="button" onClick={() => { click(); editor.deleteSelected(); }}><Trash2 size={16}/>{t("common.delete")}</button></div>;
}
...
export function RightPanel({ editor, tool, visibility, zoningOpacity, onZoningOpacity, onToggleLayer, t }: Props) {
  ...
  return <aside className="right-floating-ui">
    ...
    {(road || node || zone || building || facility) && <section className="floating-panel properties-popover glass-panel"><header><strong>{facility ? t("properties.facility") : zone ? t("properties.zone") : building ? t("properties.building") : road ? t("properties.road") : t("properties.node")}</strong><button type="button" onClick={() => editor.select(null)}><X size={16}/></button></header>
      {facility ? <FacilityPropertyForm editor={editor} facility={facility} t={t} click={click}/> : zone ? <ZonePropertyForm editor={editor} zone={zone} readOnly={tool === "select"} t={t} click={click}/> : building ? <BuildingPropertyForm editor={editor} building={building} t={t} click={click}/> : road && roadEdge ? <RoadPropertyForm editor={editor} road={road} edgeId={roadEdge.id} edgeName={roadEdge.name} groupSize={groupSize} structure={structure} t={t} click={click}/> : node && <div className="property-form"><p>{t("properties.coordinates")}: {node.x.toFixed(1)}, {node.y.toFixed(1)}</p><button className="danger-action" type="button" onClick={() => { click(); editor.deleteSelected(); }}><Trash2 size={16}/>{t("common.delete")}</button></div>}
    </section>}
  </aside>;
}
