...
import type { RoadContextMenu, RoadToolSettings, ZoneContextMenu, ZoneToolSettings } from "../../map/MapViewport";
...
interface Props {
  editor: Editor; layers: LayerVisibility; tool: EditorTool; road: RoadToolSettings; zone: ZoneToolSettings; shortcuts: KeyboardShortcuts; inputEnabled: boolean; mapRef: RefObject<MapCanvasHandle | null>;
  onZoomChange: (percent: number) => void; validation?: TranslationKey; onValidation: (key?: "road.invalid.water" | "road.invalid.short" | "zone.noRoadArea") => void; t: (key: TranslationKey) => string;
}
export function MapWorkspace(props: Props) {
  ...
  const [roadMenu, setRoadMenu] = useState<RoadContextMenu>(); const [zoneMenu, setZoneMenu] = useState<ZoneContextMenu>(); const [measurement, setMeasurement] = useState<{ x: number; y: number; text: string }>();
  ...
  return <main className="map-stage">
    <MapCanvas ref={props.mapRef} editor={props.editor} layers={props.layers} tool={props.tool} road={props.road} zone={props.zone} shortcuts={props.shortcuts} inputEnabled={props.inputEnabled} onZoomChange={(percent, scale) => { props.onZoomChange(percent); setPixelsPerMeter(scale); }} onRotationChange={onRotation} onCameraChange={setCamera} onValidation={props.onValidation} onRoadContextMenu={(menu) => { setRoadMenu(menu); if (menu) setZoneMenu(undefined); }} onZoneContextMenu={(menu) => { setZoneMenu(menu); if (menu) setRoadMenu(undefined); }} onRoadMeasurement={setMeasurement}/>
    ...
    <button className="compass-control" type="button" title={props.t("map.compass")} onClick={() => { soundManager.playClick(); props.mapRef.current?.northUp(); }}><div ref={compassNeedle} className="compass-needle"><b>N</b><i/></div></button>
    <div className="map-controls"><button type="button" title={props.t("map.fit")} onClick={() => { soundManager.playClick(); props.mapRef.current?.resetView(); }}><Focus size={18}/></button><button type="button" title={props.t("map.zoomIn")} onClick={() => props.mapRef.current?.zoomIn()}><Plus size={20}/></button><button type="button" title={props.t("map.zoomOut")} onClick={() => props.mapRef.current?.zoomOut()}><Minus size={20}/></button></div>
