...
export class Editor {
  ...
  public updateBuilding(id: string, changes: Partial<Omit<Building, "id" | "x" | "y">>): void {
    const city = this.state.city; const current = city.buildings.find((building) => building.id === id); if (!current) return; const before = structuredClone(city.buildings); const after = before.map((building) => building.id === id ? { ...building, ...changes } : building); this.commands.execute(new BuildingSnapshotCommand(changes.description !== undefined ? "Change building description" : "Update building", city, before, after, () => this.emit("buildings"))); this.emit("history"); this.emit("selection");
  }
