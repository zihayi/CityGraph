...
describe("Editor road endpoint merging", () => {
  ...
  it("updates and deletes building stories with undo", () => { const value = city(); value.buildings.push({ id: "landmark", x: 30, y: 40, width: 20, height: 15, rotation: 0, type: "public", name: "Hall" }); const editor = new Editor(value); editor.select({ kind: "building", id: "landmark" }); editor.updateBuilding("landmark", { description: "The clock rings at noon" }); expect(editor.state.city.buildings[0]?.description).toBe("The clock rings at noon"); editor.deleteSelected(); expect(editor.state.city.buildings).toHaveLength(0); editor.undo(); expect(editor.state.city.buildings[0]).toMatchObject({ id: "landmark", description: "The clock rings at noon" }); });
});
