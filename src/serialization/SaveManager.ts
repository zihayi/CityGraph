...
import { createEmptyUniversity, facilityDefaultColor, type AlumniCompany, type BusLine, type BusPathStep, type BusStop, type BusTerminal, type City, type FacilityPOI, type MapSize, type Road, type RoadCategory, type RoadEdge, type RoadNode, type RoadStructure, type RoadSubtype, type TerrainType, type University, type UniversityType, type WaterArea, type Zone } from "../model/City";
...
interface ZonesDocument { universities: University[]; zones: Zone[] }
...
function isUniversityProfile(value: unknown): boolean { return isRecord(value) && typeof value.englishName === "string" && typeof value.emblemDataUrl === "string" && (value.emblemDataUrl === "" || /^data:image\/(?:png|jpeg|webp);base64,/i.test(value.emblemDataUrl)) && typeof value.motto === "string" && (value.foundedYear === null || typeof value.foundedYear === "number" && Number.isInteger(value.foundedYear)) && typeof value.universityType === "string" && isStringArray(value.alumniCompanies) && isStringArray(value.colleges) && isStringArray(value.laboratories); }
function isImageReference(value: unknown): value is string { return typeof value === "string" && (value === "" || value.startsWith("asset:") || /^data:image\/(?:png|jpeg|webp);base64,/i.test(value)); }
...
function isFacility(value: unknown): value is SavedFacility { return isRecord(value) && typeof value.id === "string" && typeof value.type === "string" && Boolean(value.type) && typeof value.name === "string" && typeof value.icon === "string" && (value.universityZoneId === undefined || typeof value.universityZoneId === "string") && (value.affiliatedUniversityId === undefined || typeof value.affiliatedUniversityId === "string") && (value.universityAffiliationKind === undefined || value.universityAffiliationKind === "hospital" || value.universityAffiliationKind === "facility") && (value.color === undefined || typeof value.color === "string" && /^#[0-9a-f]{6}$/i.test(value.color)) && isPoint(value.position); }
...
export class SaveManager {
  ...
  private createDocuments(city: City, camera: CameraState, saveName: string, createdAt = this.createdAt, autosave = false): { metadata: MetadataDocument; map: MapDocument; roads: RoadsDocument; zones: ZonesDocument; buildings: BuildingsDocument; facilities: FacilitiesDocument } {
    ...
    return {
      ...
      zones: { universities: city.universities, zones: city.zones },
  ...
  private parse(metadataValue: unknown, mapValue: unknown, roadsValue: unknown, zonesValue: unknown = { zones: [] }, buildingsValue?: unknown, facilitiesValue?: unknown): LoadedSave {
    ...
    if (![2, 3, 4, 5, 6, 7, 8, 9, FORMAT_VERSION].includes(version)) throw new SaveError("version", version);
    ...
    const hasUniversityCollection = Object.prototype.hasOwnProperty.call(zonesValue, "universities"); const savedZones = Array.isArray(zonesValue.zones) ? zonesValue.zones : []; if (!savedZones.every(isZone) || new Set(savedZones.map((zone) => zone.id)).size !== savedZones.length) throw new SaveError("invalid");
    ...
    else { if (version >= FORMAT_VERSION) throw new SaveError("invalid"); ({ universities, zones } = migrateLegacyUniversities(savedZones as Zone[])); }
    ...
    if (busLines.some((line) => (line.loop ? line.startTerminalId !== undefined || line.endTerminalId !== undefined || line.stopIds.length < 2 : !line.startTerminalId || !line.endTerminalId || !terminalIds.has(line.startTerminalId) || !terminalIds.has(line.endTerminalId)) || new Set(line.stopIds).size !== line.stopIds.length || line.path.some((step) => !edgeIds.has(step.roadEdgeId)) || line.stopIds.some((stopId) => !busStopIds.has(stopId) || busStops.find((stop) => stop.id === stopId)?.lineId !== line.id))) throw new SaveError("invalid");
    if (busStops.some((stop) => !busLineIds.has(stop.lineId) || !edgeIds.has(stop.roadEdgeId) || !busLines.find((line) => line.id === stop.lineId)?.stopIds.includes(stop.id) || busStopPathPosition(busLines.find((line) => line.id === stop.lineId)!.path, stop) === undefined)) throw new SaveError("invalid");
