...
import type { City, MapSize, Road, RoadCategory, RoadEdge, RoadNode, RoadStructure, RoadSubtype, TerrainType, WaterArea, Zone } from "../model/City";
...
export class SaveManager {
  ...
  public async load(): Promise<LoadedSave> {
    if (isTauri()) {
      try {
        ...
        const files = await invoke<{ metadata: string; map: string; roads: string; zones: string }>("load_city_files", { folderPath: folder });
        const metadata = JSON.parse(files.metadata) as unknown; const map = JSON.parse(files.map) as unknown; const roads = JSON.parse(files.roads) as unknown; const zones = JSON.parse(files.zones) as unknown;
        const loaded = this.parse(metadata, map, roads, zones);
  ...
  private createDocuments(city: City, camera: CameraState, saveName: string, createdAt = this.createdAt, autosave = false): { metadata: MetadataDocument; map: MapDocument; roads: RoadsDocument; zones: ZonesDocument } {
    ...
    return {
      ...
      map: { mapSize: city.mapSize, worldBounds: city.bounds, terrain: city.terrain, water: city.waters, camera, buildings: city.buildings, blocks: city.blocks, parks: city.parks, pois: city.pois, transitLines: city.transitLines, transitStations: city.transitStations, labels: city.labels },
      roads: { roadNodes: city.roadNodes, roads: city.roads, roadEdges: city.roadEdges },
      zones: { zones: city.zones },
    };
  }

  private async writeDocuments(directory: FileSystemDirectoryHandle, documents: ReturnType<SaveManager["createDocuments"]>): Promise<void> { await Promise.all([this.writeJson(directory, "metadata.json", documents.metadata), this.writeJson(directory, "map.json", documents.map), this.writeJson(directory, "roads.json", documents.roads), this.writeJson(directory, "zones.json", documents.zones)]); await directory.getDirectoryHandle("assets", { create: true }); }
  private async invokeDesktopWrite(parentPath: string, folderName: string, documents: ReturnType<SaveManager["createDocuments"]>): Promise<void> { await invoke<string>("save_city_files", { parentPath, folderName, metadata: JSON.stringify(documents.metadata, null, 2), map: JSON.stringify(documents.map, null, 2), roads: JSON.stringify(documents.roads, null, 2), zones: JSON.stringify(documents.zones, null, 2) }); }
  ...
  private parse(metadataValue: unknown, mapValue: unknown, roadsValue: unknown, zonesValue: unknown = { zones: [] }): LoadedSave {
    ...
    if (version !== 2 && version !== 3 && version !== 4 && version !== FORMAT_VERSION) throw new SaveError("version", version);
    ...
    const city: City = {
      ...
      buildings: Array.isArray(mapValue.buildings) ? mapValue.buildings as City["buildings"] : [],
